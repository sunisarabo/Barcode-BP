...
- **ส่งเข้า Google Sheet อัตโนมัติ** — ตั้ง URL ของ Google Apps Script ไว้ใน
  `config.js` (`window.SHEET_URL`) แล้วแอปจะรวม log ส่งเองทุก ~15 วินาที ไปที่ Sheet
  ที่จัดข้อมูล **ตามปี-เดือน (แท็บ) + ตามเที่ยวบิน (กลุ่ม) + แท็บสรุป** — ดู [`../gas/README.md`](../gas/README.md)
  · ตั้ง `SHEET_URL` เป็นค่าว่างเพื่อปิดการส่ง
